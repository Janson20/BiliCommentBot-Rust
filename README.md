...
- ⚙️ **配置热更新**：修改后立即生效无需重启
- 📜 实时日志查看 + 分级别过滤 + 搜索 + 导出
- 📋 回复历史记录查看 + 按日期分组 + 清除（SQLite 存储，首次自动迁移旧 JSON 数据）
- 🔔 **异常提醒横幅**：B站登录失效、连续请求失败等问题会在界面上常驻提示，不再只写进日志（机器人跑一晚上没人看着时尤其重要）
- 🔒 登录密码保护（启动锁屏，bcrypt 哈希，兼容旧版 SHA-256 自动升级）
- 🖥️ **系统托盘常驻**：关闭窗口时可选择「最小化到托盘」让机器人继续后台运行。托盘菜单提供 **显示主窗口 / 启动机器人 / 停止机器人 / 立即检查 / 退出程序**，不必打开窗口也能控制
...
- **新增异常提醒横幅**：B站登录失效、Cookie 缺少 `bili_jct`、连续请求失败（默认 5 次）等问题会通过新的 `alert` 事件推到界面常驻横幅，同时写入日志。此前这些情况只体现为日志里的一行 ERROR，机器人静默失败一整晚也不会有人发现。
- **托盘菜单可直接控制机器人**：新增「启动机器人 / 停止机器人 / 立即检查」，配合开机自启静默进托盘的场景，不必再打开窗口点按钮。
- **收窄前端权限**：`tauri.conf.json` 的 allowlist 去掉了未使用的 `fs`（此前是 `**` 全盘读写）、`path`、`process`（可重启/退出进程）、`clipboard`、`protocol`；`dialog` 由 `all` 收窄为 `open`，并同步精简 `Cargo.toml` 里的 tauri features。前端只用到 `invoke` / `event` / `dialog.open`，其余都属于多余的攻击面。
- **手改配置不再丢字段**：`RawConfig` 增加未知字段兜底（`#[serde(flatten)]`），手写进 `config.toml` 的自定义 section / 字段在界面保存后不会被静默抹掉（Python 版的行为）。
- **回复长度主动截断**：超过 B站 评论上限（1000 字符）的 AI 回复会被截断并记录警告，而不是提交后被服务端拒绝。
- **🔐 安全修复：数据不再写入工作目录**（重要）
